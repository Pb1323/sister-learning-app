  ...
  return (
    <MagicWorld reducedMotion={settings.reducedMotion} tint={item.softColor}>
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.letterTrail}>{alphabet.map((trailLetter, trailIndex) => <View key={trailLetter.letter} style={[styles.trailDot, trailIndex === index && { backgroundColor: item.color, transform: [{ scale: 1.35 }] }]} />)}</View>
...
const styles = StyleSheet.create({
  ...
  letterTrail: { alignSelf: 'center', maxWidth: '92%', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 5, marginTop: 10 },
  trailDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: 'rgba(255,255,255,0.85)', borderWidth: 1, borderColor: '#fff' },
